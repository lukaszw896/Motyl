...
{
	XMMATRIX rotationMtx = XMMatrixRotationX(XM_PI/2);
	XMMATRIX translationMtx = XMMatrixTranslation(0,-DODECAHEDRON_H/2.0f,0);
	m_dodecahedronMtx[0] = rotationMtx * translationMtx;


	m_dodecahedronMtx[1] = XMMatrixRotationX(-XM_PI/2)*XMMatrixTranslation(DODECAHEDRON_R,0,0)*XMMatrixRotationZ(DODECAHEDRON_A)*XMMatrixTranslation(-DODECAHEDRON_R,-DODECAHEDRON_H/2,0);

	for(int i=0;i<4;i++){
	m_dodecahedronMtx[2+i] = m_dodecahedronMtx[1+i] * XMMatrixRotationY(2*XM_PI/5);
	}
